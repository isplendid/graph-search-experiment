...
do
  ...
  do
    ...
    timeout -s 9 180s java -Xmx4096m -cp bin:lib/nxparser.jar:lib/je-3.3.69.jar:lib/lidaq_web_repos.jar:lib/openrdf-sesame-2.3.2-onejar.jar sjtu.apex.gse.exp.ExperimentSrcNo $cfg_file $query $result_file $output_file > /dev/null 2>&1
  done
done
